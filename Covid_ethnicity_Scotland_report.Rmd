...
Data were extracted for this analysis on 8th June 2020. They do not include the Lighthouse data as we do not have access to this through ECOSS as yet. 
...
As other sociodemographic factors may vary by ethnicity we further adjusted analyses for residential care home status, Scottish Index of Multiple Deprivation (SIMD) and diabetes. 

# Results
SMR based ethnicity was unavailable for approximately 25% of cases and 35% of controls. The ONOMAP approach  assigned ethnicity  in >99% of persons. 


## Agreement between ethnicity assigned by name classification (ONOMAP) and self-reported ethnicity in SMR records

Table \ref{tab:crosstab} shows the agreement between the ONOMAP  (name based) assigned ethnicity and the self-assigned SMR ethnicity.  The name classification assigns to the same as  SMR category `r table.ethnic[1, 1]` of those who self-report their ethnicity as White, `r table.ethnic[4, 4]` of those who self-report as South Asian, `r table.ethnic[3, 3]` of those who self-report as Chinese, but only `r table.ethnic[2, 2]` of those who self-report as Black.  The proportion with  non-White ethnicity  is low for all groups. Thus although the sensitivity of ONOMAP for assigning SMR South Asian ethnicity is >99% the positive predictive value of ONOMAP assigned South Asian ethnicity is just 65%.

Clearly analyses that distinguish the Black group can be done only for the subset of individuals who have SMR records. However, the differential missingness between cases and controls means the rate ratios here must be interpreted with caution.  Therefore, in all further analyses, the ONOMAP classification has been collapsed to four groups: White, South Asian, Chinese, Other.  
...
There is evidence of an elevated risk of  "severe test positive disease" for those of South Asian ethnicity using both the SMR and ONOMAP approach (univariate rate ratios of 1.58 and 1.45 respectively) with borderline significance.  The rate ratios increase and are slightly more significant when SIMD and residential care home status are adjusted for (adjusted rate ratios of 1.68 and 1.59 using SMR and ONOMAP respectively).They are attenuated and lose significance when further adjustment is made for diabetes status (rate ratios of 1.44 and 1.38 using SMR and ONOMAP respectively).

There is also some evidence of an elevation in risk of "severe test positive or death" among those of Black and South Asian ethnicity.  The univariate rate ratio among those of Black ethnicity is 1.12 and increases to 1.38 after adjustment for SIMD and residential care home status.  This effect is observed using the SMR approach since ONOMAP cannot be used to assign Black ethnicity as we describe above. Further adjustment for diabetes status reduces the rate ratio to 1.25.

For those of South Asian ethnicity, the rate ratios associated with "severe test positive or death" using the SMR approach are lower than for "severe test positive disease" and are not significant (univariate rate ratio is 1.23 and rate ratio after adjustment for SIMD and residential care home status is 1.42).  Using the named based classification, for those of South Asian ethnicity the univariate rate ratio is 1.16 and is not significant.  Adjustment for SIMD and residential care home status increases the rate ratio to 1.4.  Further adjustment for diabetes status reduces the rate ratios, using SMR and ONOMAP, to 1.24.

Using the SMR approach, there are significant increased rate ratios for those of Black and South Asian ethnicity for test positivity. However note our caveats on interpretation of this - both because of differential missingness in SMR based ethnicity and because of selection effects on test positive levels.  

Significant increased rate ratios for "hospital or severe disease" were observed among those of South Asian ethnicity using both SMR and ONOMAP approaches.

For those of Chinese ethnicity, lower test positivity was observed using the SMR and ONOMAP approaches.  Further to this, using SMR data, there was also a reduced risk of "severe test positive disease" and "severe test positive or death" among this ethnic group. 
...
There are borderline significant elevations in rates of having a positive test, being test-positive and hospitalised or being test-positive and severe (CCU or death) rates in those of South Asian origin.  When deaths with a mention of COVID in those never tested positive are included as fatal cases there is no significant elevation in risk ratio in those of South Asian origin compared to Whites.  Where elevated risk ratios are seen the are increased by adjusting for SIMD and residential care home status so lower socioeconomic status and care home status does not explain these elevations.  Further adjustment for diabetes attenuated the risk ratio consistent with higher prevalence of diabetes contributing to the elevations in risk.

For those of Black ethnic origin, there was evidence of elevation in risk of having a positive test and being test-positive and being hospitalised but not of severe disease or severe disease with additional death registrations in those never tested positive included.

For those of Chinese origin here was evidence of a reduction in risk of having a positive test compared to whites.  Point estimates for hospitalised and severe disease risk ratios were well below 1 but not significantly.  Absolute numbers were very low for this ethnic group.
...
```{r ethnic5smrDB, echo=FALSE, warning=FALSE, message=FALSE}

rownames(table.ethnic5smrDB) <- gsub("_", " ", rownames(table.ethnic5smrDB))
knitr::kable(table.ethnic5smrDB,
             escape=FALSE, 
             booktabs=TRUE,
             label="ethnicsmrDB",
             col.names=c(clean.header(colnames(table.ethnic5smrDB)[1:2]),
                         "Rate ratio (95\\% CI)", "p-value",
                         "Rate ratio (95\\% CI)", "p-value"),
             align=rep("c", 6), 
             caption="Associations of severe disease among test positives with ethnicity (additional adjustment for Diabetes) in those with ethnicity recorded in Scottish Morbidity record") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Multivariate"=2)) %>%
    kable_styling(latex_options="hold_position") %>% 
    column_spec(column=1, width="2.7cm") %>%
    column_spec(column=2, width="1.85cm")  %>%
...
```{r ethnic5smrNRSDB, echo=FALSE, warning=FALSE, message=FALSE}

rownames(table.ethnic5smrNRSDB) <- gsub("_", " ", rownames(table.ethnic5smrNRSDB))
knitr::kable(table.ethnic5smrNRSDB,
             escape=FALSE, 
             booktabs=TRUE,
             label="ethnicsmrNRSDB",
             col.names=c(clean.header(colnames(table.ethnic5smrNRSDB)[1:2]),
                         "Rate ratio (95\\% CI)", "p-value",
                         "Rate ratio (95\\% CI)", "p-value"),
             align=rep("c", 6), 
             caption="Associations of severe disease OR a COVID-19 death with ethnicity (additional adjustment for Diabetes) in those with ethnicity recorded in Scottish Morbidity record") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Multivariate"=2)) %>%
    kable_styling(latex_options="hold_position") %>% 
    column_spec(column=1, width="2.7cm") %>%
    column_spec(column=2, width="1.85cm")  %>%
...
```{r onomapDB, echo=FALSE, warning=FALSE, message=FALSE}

rownames(table.severe.demogDB) <- gsub("_", " ", rownames(table.severe.demogDB))
knitr::kable(table.severe.demogDB,
             escape=FALSE, 
             booktabs=TRUE,
             label="onomapDB",
             col.names=c(clean.header(colnames(table.severe.demogDB)[1:2]),
                         "Rate ratio (95\\% CI)", "p-value",
                         "Rate ratio (95\\% CI)", "p-value"),
             align=rep("c", 6),
             caption="Associations of severe disease among test positives with demographic variables (additional adjustment for Diabetes) including ethnicity based on name classification") %>%
    kable_styling(latex_options="hold_position") %>% 
    column_spec(column=1, width="2cm") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Multivariate"=2)) %>%
    column_spec(column=2, width="1.85cm")  %>%
...
```{r onomapNRSDB, echo=FALSE, warning=FALSE, message=FALSE}

rownames(table.severe.demogNRSDB) <- gsub("_", " ", rownames(table.severe.demogNRSDB))
knitr::kable(table.severe.demogNRSDB,
             escape=FALSE, 
             booktabs=TRUE,
             label="onomapNRSDB",
             col.names=c(clean.header(colnames(table.severe.demogNRSDB)[1:2]),
                         "Rate ratio (95\\% CI)", "p-value",
                         "Rate ratio (95\\% CI)", "p-value"),
             align=rep("c", 6),
             caption="Associations of severe disease OR any COVID-19 death with demographic variables (additional adjustment for Diabetes) including ethnicity based on name classification") %>%
    kable_styling(latex_options="hold_position") %>% 
    column_spec(column=1, width="2cm") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Multivariate"=2)) %>%
    column_spec(column=2, width="1.85cm")  %>%
