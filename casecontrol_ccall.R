...
setwd("~/covid-scotland")
source("helperfunctions.R")
setwd("/data/HPS/Covid19/Case_control")
...
##count NRS deaths as deathswithin28==TRUE
cc.all[cc.all$CASE==1&cc.all$testpositive.case==FALSE,]$deathwithin28<-TRUE
...
setwd("~/Amanda files")
source("ethnic_assign_withPartB.R")
setwd("~/covid-scotland")
...
cc.all$ethnic5.onomap <- factor(cc.all$ethnic5.onomap, levels=levels(cc.all$ethnic5.onomap)[c(1,2,3,5,4)])
...
### Recode ethnicity into 9 SMR groups and 8/7 ONOMAP codes for dissagreggated tables

cc.all <- mutate(cc.all, ethnic9.smr = collapseto9.ethnicsmr(ETHNIC_SMR_LAST))

cc.all <- mutate(cc.all,
                 group.onomap2 = group.onomap2(OnolyticsType, GeographicalArea))
cc.all <- mutate(cc.all,
...
## AMANDA CODE TO CHECK DIABETES DATA
cc.all$dm.type2<-cc.all$dm.type
cc.all[cc.all$dm.type==0&cc.all$diab.reg==1,]$dm.type2<-999
cc.all$diab2020<-rep(0,length(cc.all[,1]))
cc.all[cc.all$diab.reg==1&cc.all$dm.type2==999,]$diab2020<-1
...
cc.severe <- cc.all[casegroup=="A" | casegroup=="D",]
...
##add in cc.all
bnfcols <- grep("^BNF", colnames(cc.all))
for(j in bnfcols) {
  cc.all[[j]][is.na(cc.all[[j]])] <- 0
  cc.all[[j]][cc.all[[j]] > 1] <- 1
  cc.all[[j]] <- as.factor(cc.all[[j]])
}
...
x <- apply(subset(cc.all, select=subparacols), 2, as.character)
...
table(cc.all$numdrugsgr)

cc.all$numdrugs.notppi <- cc.all$numdrugs.subpara - cc.all$y.protonpump
cc.all$numdrugs.notppi.gr <- 3 * ceiling(cc.all$numdrugs.notppi / 3)
cc.all$numdrugs.notppi.gr <- as.factor(car::recode(cc.all$numdrugs.notppi.gr, "15:hi='>12'"))
cc.all[, numdrugs.notppi.gr := factor(numdrugs.notppi.gr,
                                      levels=levels(numdrugs.notppi.gr)[c(2, 4:6, 3, 1)])]
table(cc.all$numdrugs.notppi.gr)

cardiovasc.subparacols <- grep("^subpara\\.2", names(cc.all))
x <- apply(subset(cc.all, select=cardiovasc.subparacols), 2, as.character)
x <- apply(x, 2, as.integer)
cc.all$numdrugs.cardiovasc <- rowSums(x)
cc.all$numdrugs.notcardiovasc <- cc.all$numdrugs.subpara - cc.all$numdrugs.cardiovasc
...
### add in cc.all
cc.all <- diagnoses.wide[cc.all]
cc.severe <- diagnoses.wide[cc.severe]

## add in cc.all
icdcols <- grep("^Ch.", colnames(cc.all))
for(j in icdcols) {
  cc.all[[j]][is.na(cc.all[[j]])] <- 0
  cc.all[[j]][cc.all[[j]] > 1] <- 1
  cc.all[[j]] <- as.factor(cc.all[[j]])
...
cc.all$num.icdchapters <-
  rowSums(matrix(as.integer(as.matrix(subset(cc.all, select=icdcols))),
                 nrow=nrow(cc.all)))
cc.all$num.icdchapters.gr <-
  as.factor(car::recode(cc.all$num.icdchapters,
                        "0='No discharge records'; 1:2='1-2 ICD-10 chapters';
                        3:hi='3 or more chapters'"))
cc.all[, num.icdchapters.gr := relevel(num.icdchapters.gr, ref="No discharge records")]
...
cc.all$listed.any <-
  as.factor(as.integer(with(cc.all,
                            diabetes.any=="Diabetic" | IHD.any==1 |
                              heart.other.any==1 |
                              ckd.any==1 | oad.any==1 |
                              neuro.any==1 | liver.any==1 | immune.any==1)))

cc.all$diag.other <- as.integer(cc.all$listed.any==0 & cc.all$diag.any==1)
cc.all$diag.other <- as.factor(car::recode(cc.all$diag.other,
