...
_Background_-- We previously reported that dispensing of prescriptions from primary care in the past year was associated with severe COVID-19 in Scotland, even in those without past diagnoses of conditions designated as increased risk (listed conditions). We report a further investigation of possible explanations for this association, which include specific co-morbidity or drug effects but also more diffuse effects of frailty or polypharmacy. 
...
*Findings* -- In those without a diagnosis of a listed condition, severe COVID-19 was strongly associated with polypharmacy; the rate ratio (95% CI) associated with dispensing of 12 or more drug classes versus no dispensed prescriptions was `r table.numdrugsgr.listed.any[6, 4]`. In those with listed conditions, this association was in the reverse direction: the corresponding rate ratio was `r table.numdrugsgr.listed.any[12, 4]`.  The association was specifically with non-cardiovascular drugs: in a model with number of cardiovascular drug classes and number of other drug classes as covariates, the rate ratios were respectively `r table.jointcardiovasc[1, 4]` and `r table.jointcardiovasc[2, 4]`. 
A backwards elimination procedure identified proton pump inhibitors and opiate analgesics as the drug classes with the largest independent contribution to the association of COVID-19 with polypharmacy.  Further analysis of these associations showed dose-response relationships that were strongest in younger age groups and reduced only slightly by adjusting for prespecified covariates.  Stratification by time window of exposure showed that the rate ratios were higher for those with prescriptions dispensed only in the last 120 days than for those with prescriptions dispensed only in an earlier time window.  

*Conclusions* -- In Scotland severe COVID-19 is strongly associated with polypharmacy. This association is not easily explained by co-morbidity as it is strongest in those without relevant medical history.  Of all drug classes, analgesics and proton pump inhibitors make the largest independent contribution to this association.  As prescriptions of these drug classes are recognised indicators for overprescribing, it is difficult to distinguish between causality and confounding as explanations for this association.  This requires further investigation in other settings.  In the meantime existing advice on reducing inappropriate polypharmacy and limiting the prescribing of opiates and proton pump inhibitors should be reinforced. 

\newpage

# Background
The REACT-SCOT study is a matched case control study of severe COVID-19 in Scotland.  In the first report of this study we reported a strong association of severe COVID-19 with having had at least one prescription dispensed in the past year [@mckeigue_rapid_2020].  The univariate rate ratios associated with at least one prescription varied from 4.4 in those aged under 60 years to 3.5 in those aged 75 years and over.   This association persisted after adjusting for care home residence, hospital admission in the last five years, and diagnoses of conditions designated as high-risk conditions for COVID-19 by the NHS and other public health agencies (hereafter listed conditions).  The objective of this study was to investigate possible explanations for this association: confounding by co-morbidity  factors, or direct effects of prescribed drugs.  
...
We began by constructing an index of polypharmacy as the number of drug classes (BNF subparagraph codes) for which at least one prescription had been dispensed during the period of observation (from 1 June 2019 to 15 days before the date of testing positive). 

Table \ref{tab:numdrugs.listed.any} shows that the COVID-19 was associated with polypharmacy in those without a listed condition, but that in those with a listed condition the direction of this association was reversed: those with dispensed prescriptions of 1 to 12 drug classes were at lower risk than those with no dispensed prescriptions. 

To study further the relationship with polypharmacy, we restricted the analysis to those without a diagnosis of a listed condition.  As prescribing of multiple drugs for primary and secondary prevention of cardiovascular disease is in accordance with evidence-based guidelines, we partitioned the number of drug classes dispensed between cardiovascular and other drugs.  In a model with number of cardiovascular drug classes and number of other drug classes as covariates, the rate ratios were respectively `r table.jointcardiovasc[1, 4]` and `r table.jointcardiovasc[2, 4]`. 
...
A backward stepwise variable elimination procedure as described above was carried out to identify which of these drug classes contributed most to the association of COVID-19 with polypharmacy.  Figure \ref{fig:stepwise} shows how the log-likelihood changed as these variables were dropped from the model. Dropping the first `r firstrow.neg - 1` drug classes increased the log-likelihood of the model.  Only for the last `r length(rows.print)` drug classes did the log-likelihood decrease when the variable was dropped.  The last drug classes retained by backward selection, and thus the variables with largest independent explanatory power for the effect of polypharmacy, were proton pump inhibitors and nonopioid or compound analgesics. 

The associations with these drug classes were examined in more depth.  For studies of these specific exposures the observations were not restricted to exclude those with listed conditions.  Distinguishing between causality and confounding is challenging because these drug classes are usually co-prescribed with other drugs.  We used several approaches:

* Testing for a dose-response relationship and stratifying by age group

* Adjusting for prespecified covariates: for proton pump inhibitors these covariates were specified as hospital diagnosis of a disorder of the esophagus, stomach or duodenum , ever-use of non-steroidal anti-inflammatory drugs, anti-platelet agents and anticoagulants.  For opiate use any history of neoplasm was included as a covariate.  Care home residence and SIMD quintile were included as covariates for both these drug classes. 
...
## Associations of severe COVID-19 with opiate use
Table \ref{tab:dosegr.opiate} shows the relationship of COVID-19 to average daily opiate dose (MMEs) over the observation period.  This showed a dose-response relationship.  With those who had not been dispensed opiates as baseline, the univariate rate ratio was XX in those with average dose daily of more than 50 mg morphine equivalent, reduced to YY on adjusting for prespecified covariates. 

Table \ref{tab:timewindow.opiate shows that the rate ratio associated with exposure in the most recent 120-day time window was higher than that associated with exposure in a less recent time window (from 1 June 2019 to 121 days before the specimen date)
...
The drug classes that are strongly associated with severe COVID-19 include drugs acting on the gastrointestinal and central nervous systems that are often overprescribed, and are used as indicators of overprescribing in the Scottish National Therapeutic Index of prescribing quality [@macbride-stewart_national_2019]. These indicators of overprescribing include proton pump inhibitors and analgesics, identifed by backwards elimination as the drug classes with the largest independent contribution to the association of COVID-19 with polypharmacy. 
Distinguishing between causality and confounding as possible explanations for the association of COVID-19 with these drug classes is difficult because drug exposures are not measured accurately, and the associations are likely to be confounded by other drugs, by co-morbidities and more generally by frailty and socioeconomic deprivation. The time of exposure to a drug does not necessarily correspond to the time intervals for which the prescription was issued.   Because proton pump inhibitors are available over the counter, dispensed prescriptions do not capture all exposure. 

For both opiates and proton pump inhibitors, four criteria provide modest evidence favouring causal explanations over confounding.  There are strong dose-response relationships of COVID-19 to dispensed average daily dose.  Adjustment for covariates pre-specified as likely to confound these associations reduces the effect size only slightly. The effect sizes are larger in younger individuals, in whom the prevalence of comorbid conditions and frailty is lower than in older individuals.  Among ever-exposed individuals, the rate ratios associated with dispensing only in the most recent time window were higher than the rate ratios associated with dispensing only in an earlier time window.  We emphasize however that this   evidence is not conclusive. It may be possible to resolve this through studies in other countries where COVID-19 epidemics have been especially severe and overprescribing of proton pump inhibitors [@luo_changes_2018; @pasina_overuse_2020; @ramirez_overuse_2010] or opiates has been reported previously.  In Scotland proton pump inhibitors are widely prescribed among the elderly, often without any clear indication [@jarchow-macdonald_prescribing_2013; @marks_time_2016; @kurdi_ongoing_2018], but overprescribing of opiates is still relatively uncommon.

The dose-response relationship of opiate use to COVID-19 is similar in magnitude to that reported for community-acquired pneumonia in a study of people receiving medical care through the Veterans Administration before 2012 [@edelman_association_2019].  As SARS-CoV-2 is at least partly an enteric infection [@wan_enteric_2020] and the ACE2 receptor is expressed in the intestine, it is plausible that proton pump inhibitors could increase the risk or severity of infection.  
...
```{r stepwise, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, out.width = "0.99\\textwidth", fig.cap="Stepwise dropping of drug classes from the polypharmacy variable: each row shows the log-likelihood when that drug class is dropped from a model with care home as covariate"}
...
```{r numdrugs.cardio, echo=FALSE, warning=FALSE, message=FALSE}

colnames(table.jointcardiovasc)[c(1, 4:5)] <- c("Drug category", "Odds ratio (95\\% CI)",
                                                "$p$-value")

knitr::kable(table.jointcardiovasc,
             escape=FALSE,
             booktabs=TRUE,
             label="numdrugs.cardio",
             row.names=FALSE,
             caption="Multivariate regression of severe COVID-19 on number of cardiovascular and number of noncardiovascular drug classes in dispensed prescriptions, among those without listed conditions") %>%
    column_spec(column=1, width="2.5cm") %>%
    add_header_above(c(" "=1, "Mean number of drug classes"=2)) %>%
    kable_styling(latex_options="hold_position")

```

```{r dosegr.opiate, echo=FALSE, warning=FALSE, message=FALSE}

knitr::kable(table.dosegr.opiate,
             escape=FALSE,
             label="dosegr.opiate", 
             booktabs=TRUE,
             row.names=FALSE, # dose levels in column 1
             col.names=c("Average MME/day", colnames(table.dosegr.opiate)[2:3], 
                         "Rate ratio (95\\% CI)", "$p$-value",
                         "Rate ratio (95\\% CI)", "$p$-value"),
             caption="Associations of severe COVID-19 with dispensed average daily dose of opiate (MME) grouped into categories, with and without adjustment for prespecified covariates") %>%
...
```{r timewin.opiate, echo=FALSE, warning=FALSE, message=FALSE}

knitr::kable(table.timewindow.opiate,
             escape=FALSE,
             label="timewindow.opiate", 
             booktabs=TRUE,
             col.names=c(clean.header(colnames(table.timewindow.opiate)[1:2]), 
                         "Rate ratio (95\\% CI)", "$p$-value"),
             caption=paste0("Associations of severe disease with dispensed prescriptions for opiate-containing analgesics by ", TW, "-day time window, in those without a history of neoplasm")) %>%
...
knitr::kable(table.dosegr.protonpump,
             ...
             col.names=c("Average DDDs/day", colnames(table.dosegr.protonpump)[2:3], 
                         "Rate ratio (95\\% CI)", "$p$-value",
                         "Rate ratio (95\\% CI)", "$p$-value"),
             caption="Associations of severe COVID-19 with dispensed average daily dose of proton pump inhibitor (DDDs) grouped into categories, with and without adjustment for prespecified covariates") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Adjusted for covariates"=2)) %>%
    kable_styling(latex_options="hold_position") %>%
    kableExtra::group_rows("Ages 0-74 years", 1, 5) %>%
    kableExtra::group_rows("Ages 75+ years", 6, 10) %>%
    landscape()
