...
*Findings* -- In those without a diagnosis of a listed condition, severe COVID-19 was strongly associated with polypharmacy, defined by total number of drug classes dispensed. With those who had no dispensed prescriptions as reference group, the rate ratio (95% CI) for COVID-19 associated with dispensing of 12 or more drug classes was `r table.numdrugsgr.listed.any[6, 4]` in those without listed conditions.  In those with listed conditions, this association was in the reverse direction: the corresponding rate ratio was `r table.numdrugsgr.listed.any[12, 4]`.  The association was specifically with non-cardiovascular drugs: in a model with number of cardiovascular drug classes and number of other drug classes as covariates, the rate ratios were respectively `r table.jointcardiovasc[1, 4]` and `r table.jointcardiovasc[2, 4]`. 
A backwards elimination procedure identified proton pump inhibitors, and analgesics (both opiates and non-opiate / compound preparations) as the drug classes with the largest independent contribution to the association of COVID-19 with polypharmacy.  Further analysis of the association with proton pump inhibitors showed a dose-response relationship that was strongest in younger age groups: the rate ratio associated with average dispensed dose of 2 or more DDDs per day was `r table.dosegr.protonpump[5, 4]` in those under 60 years and `r table.dosegr.protonpump[15, 4]` in those aged under 60 years.  

*Conclusions* -- In Scotland severe COVID-19 is strongly associated with polypharmacy. This association is not easily explained by co-morbidity as it is strongest in those without relevant medical history.  Of all drug classes, analgesics and proton pump inhibitors make the largest independent contribution to this association.  As prescriptions of these drug classes are recognised indicators for overprescribing, it is difficult to distinguish between causality and confounding as explanations for this association.  This requires further investigation in other settings.  In the meantime existing advice on reducing inappropriate polypharmacy and limiting the prescribing of proton pump inhibitors should be reinforced. 
...
## Associations of severe COVID-19 with analgesic use
...
knitr::kable(table.dosegr.opiate,
             ...
             booktabs=TRUE,
             row.names=TRUE, 
             col.names=c(clean.header(colnames(table.dosegr.opiate)[1:2]), 
                         "Rate ratio (95\\% CI)", "$p$-value",
                         "Rate ratio (95\\% CI)", "$p$-value"),
             caption="Associations of severe COVID-19 with dispensed average daily dose (MME) of opiate grouped into categoriesb") %>%
    add_header_above(c(" "=3, "Univariate"=2, "Multivariate"=2)) %>%
    kableExtra::group_rows("Average dispensed daily dose (MME) of opiates", 1, 4) %>%
        kable_styling(latex_options="hold_position") %>%
        landscape()
...
knitr::kable(table.timewindow.opiate,
             ...
             col.names=c(clean.header(colnames(table.timewindow.opiate)[1:2]), 
                         "Rate ratio (95\\% CI)", "$p$-value"),
             caption=paste0("Associations of severe disease with dispensed prescriptions for opiate analgesics by ", TW, "-day time window")) %>%
...
knitr::kable(table.dosegr.protonpump,
    ...
    kable_styling(latex_options="hold_position") %>%
    kableExtra::group_rows("Ages 0-59 years", 1, 5) %>%
    kableExtra::group_rows("Ages 60-74 years", 6, 10) %>%
    kableExtra::group_rows("Ages 75+ years", 11, 15) %>%
    landscape()
