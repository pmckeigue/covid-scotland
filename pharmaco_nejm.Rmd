...
# rmarkdown::render("pharmaco_nejm.Rmd", output_file="pharmaco.pdf")
...
Dropping the first `r firstrow.neg - 1` drug classes increased the log-likelihood of the model.  The drug classes dropped at this stage included some of the most widely-prescribed drug classes that were associated with COVID-19 in univariate analyses, including lipid-lowering drugs, non-steroidal antiinflammatory agents and angiotensin-converting enzyme (ACE) inhibitors.  Only for the last `r length(rows.print)` drug classes did the log-likelihood decrease when the variable was dropped.  The `r length(rows.print)` drug classes retained at this stage included loop diuretics, calcium channel blockers, and non-opioid analgesics.  The last drug class retained by backward selection, and thus the variable with largest independent explanatory power for the effect of polypharmacy, was proton pump inhibitors. 

As the association with proton pump inhibitors was not obviously attributable to confounding by conditions designated as risk conditions for COVID-19, we explored it in more depth.  For these analyses the dataset was not restricted to exclude care home residents or those diagnosed with a listed condition as we were examining the effects of a specific drug class rather than polypharmacy.  We used several approaches to distinguish between causality and confounding:

* Testing for a dose-response relationship 

* Adjusting for prespecified covariates: care home residence, quintile of index of socioeconomic deprivation, hospital diagnosis of a disorder of the esophagus, stomach or duodenum , ever-use of non-steroidal anti-inflammatory drugs, anti-platelet agents and anticoagulants. 
...
```{r stepwise, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=5, fig.cap="\\label{fig:stepwise}Stepwise dropping of drug classes from the polypharmacy variable: each row shows the coefficient of the polypharmacy variable with that variable retained, and the log-likelihood when that drug class is dropped"}
