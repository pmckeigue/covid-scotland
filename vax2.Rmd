...
- Examination of  vaccine efficacy among those eligible for shielding broken down by the reason for shielding showed that for  solid organ transplant recipients the number of cases is still too small for the efficacy of vaccination to be estimated.  For all other groups there was a statistically significant reduction in hospitalisation for COVID-19 associated with  vaccination and that the magnitude of this reduction was similar to that in those who were not eligible for shielding. 
- Further data in larger populations are needed to evaluate vaccine efficacy in solid organ transplant recipients.
- Until such data are available it should not be assumed that vaccination is efficacious in this group.
...
Supplementary Table \ref{tab:freqsshieldgroups} shows  the vaccination status of  severe cases and  their controls  by risk stratum.  . Supplementary Table \ref{tab:freqshosp} shows the same breakdown for hospitalised cases and controls.   These   tables show that the numbers of cases in some shielding subgroups, especially solid organ transplant recipients, are too small for rate ratios to be estimated reliability. <!--delete the misleading crude odds ratios from these tables  alter the tables as discussed AND CHANGE THE TITLES ACCORDNGLY-->
...
For more detail on the computation of the vaccine  efficacy in a matched case control design see  the statistical note in the supplement and the associated  Supplementary Table \ref{tab:mhdates} <!--this comes out as s table  4 but there is no S table 3 please rename--> . 
...
Since we have previously shown that  the shielding group overall have approximately 6 fold higher risk of severe COVID-19 than those of the same age and sex without risk conditions [@mckeigue_relation_2021-1] they would need a vaccine efficacy of  82% (i.e. a rate ratio of 0.18)  with a single dose to achieve a lowering of risk to the level of unvaccinated persons  without risk conditions.   Thus whilst those in shielding  can be reassured that the vaccine is showing efficacy they should also be advised that they remain at increased risk  after a single dose (about twofold compared to those of the same age and sex without risk conditions given the vaccine-associated rate ratio of about 0.3). Thus they should remain vigilant  until they have received their second dose and background transmission rates are low. 

Also a  larger dataset than the  Scottish population is needed to estimate efficacy in solid organ transplant recipients where we have not been able to demonstrate efficacy. It is important that  such an analysis be conducted so that such persons can be advised appropriately as to their ongoing risk level. We note that such data  would also be important to understand the need for development of potential alternatives  to vaccines such as synthetic antibodies that are now being evaluated in clinical trials [@eli_lilly_and_company_phase_2021]
...
Each matched set can be represented as a 2 \ensuremath{\times} 2 table, in which the numbers in each cell are traditionally labelled $a, b, c, d$: 
 ...
 All | $C_1$ | $C_0$ | $N$ |
--->
Put simply, the vaccine efficacy is estimated as the most likely value of the odds ratio associated with vaccination given the observed values in each matched set, conditional on the marginal totals (total cases, total controls, total vaccinated, total unvaccinated.  The log-likelihood of the odds ratio given the data is summed across all sets: matched sets in which all or none are vaccinated  do not contribute to the log-likelihood. So for example whilst it is obvious that sets that pertain to days before the vaccination programmes started cannot be informative it may be less obvious that sets pertaining to days when vaccination would be 100% complete are also uninformative. The calendar time period that is most informative is that when around 50% of the population   under study is vaccinated. 

When estimating the efficacy within a risk group, only cases and controls in that risk group are retained in the matched sets, and only those only matched sets that contain both unvaccinated and vaccinated individuals contribute to the likelihood. 

It follows from the above that crude tables of the vaccination status of cases and controls in each risk group cannot be used to estimate the efficacy.  This is especially true given that vaccination rates are rising sharply over the study period. 

Table \ref{tab:mhdates} may give the reader a more intuitive  sense of how the information for computing vaccine efficacy in the shielded evolved over the study period.  It shows the weekly observed split of vaccinated versus unvaccinated cases among those in shielding versus the expected split conditional upon the marginal totals.  To keep the working simple, this table is restricted to those 778 cases eligible for shielding who  were not resident in care homes so there is no covariate adjustment.  The variance of the expected can be interpreted as the information about vaccine efficacy. 
